...
public class AnalyticsController {
    ...
    public ResponseEntity<Map<String, Object>> getOverallAnalytics(@PathVariable("id") Long id,
        ...
        return ResponseEntity
                .ok(analyticsService.getOverallAnalytics(id, userDetails.getId(), checkIsAdmin(userDetails)));
    }
    ...
    public ResponseEntity<Map<String, Long>> getGeoAnalytics(@PathVariable("id") Long id,
            @AuthenticationPrincipal UserDetailsImpl userDetails) {
        return ResponseEntity.ok(analyticsService.getGeoAnalytics(id, userDetails.getId(), checkIsAdmin(userDetails)));
    }
    ...
    public ResponseEntity<Map<String, Long>> getDeviceAnalytics(@PathVariable("id") Long id,
            @AuthenticationPrincipal UserDetailsImpl userDetails) {
        return ResponseEntity
                .ok(analyticsService.getDeviceAnalytics(id, userDetails.getId(), checkIsAdmin(userDetails)));
    }
    ...
    public ResponseEntity<Page<com.urlshortener.entity.ClickEvent>> getLinkEvents(@PathVariable("id") Long id,
        ...
        return ResponseEntity.ok(analyticsService.getLinkEvents(id, userDetails.getId(), checkIsAdmin(userDetails), query, pageable));
    }
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    @GetMapping("/stats")
    public ResponseEntity<Map<String, Object>> getDashboardStats(@AuthenticationPrincipal UserDetailsImpl userDetails) {
    ...
    public ResponseEntity<org.springframework.core.io.Resource> exportLinkAnalytics(@PathVariable("id") Long id,
            @AuthenticationPrincipal UserDetailsImpl userDetails) {
        if (!checkIsAdmin(userDetails) && !com.urlshortener.entity.Plan.PRO.name().equals(userDetails.getPlan())) {
            throw new org.springframework.security.access.AccessDeniedException(
                    "Exporting analytics is a Pro feature. Please upgrade your plan.");
        }

        byte[] pdfContent = analyticsService.exportLinkAnalytics(id, userDetails.getId(), checkIsAdmin(userDetails));
